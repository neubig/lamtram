...
BOOST_AUTO_TEST_CASE(TestWriteRead) {
  ...
  NeuralLM exp_lm(exp_vocab, 2, 2, false, 3, BuilderSpec("rnn:2:1"), -1, "full", false,*exp_mod);
  // Write the LM
  ostringstream out;
...
BOOST_AUTO_TEST_CASE(TestDecodingScores) {
  ...
  NeuralLMPtr lmptr(new NeuralLM(vocab, 1, 0, false, 3, BuilderSpec("rnn:2:1"), -1, "full", false,*mod));

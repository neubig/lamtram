...
cnn::expr::Expression SoftmaxHinge::CalcLoss(cnn::expr::Expression & in, cnn::expr::Expression & prior, const Sentence & ngram, bool train) {
...
cnn::expr::Expression SoftmaxHinge::CalcLoss(cnn::expr::Expression & in, cnn::expr::Expression & prior, const std::vector<Sentence> & ngrams, bool train) {
  Expression score = affine_transform({i_sm_b_, i_sm_W_, in});
  if(prior.pg != nullptr) score = score + prior;
...
cnn::expr::Expression SoftmaxHinge::CalcProb(cnn::expr::Expression & in, cnn::expr::Expression & prior, const Sentence & ctxt, bool train) {
  THROW_ERROR("CalcProb not valid for hinge");
  return softmax(affine_transform({i_sm_b_, i_sm_W_, in}));
}
cnn::expr::Expression SoftmaxHinge::CalcProb(cnn::expr::Expression & in, cnn::expr::Expression & prior, const vector<Sentence> & ctxt, bool train) {
  THROW_ERROR("CalcProb not valid for hinge");
  return softmax(affine_transform({i_sm_b_, i_sm_W_, in}));
}
cnn::expr::Expression SoftmaxHinge::CalcLogProb(cnn::expr::Expression & in, cnn::expr::Expression & prior, const Sentence & ctxt, bool train) {
  return (prior.pg != nullptr ?
          affine_transform({i_sm_b_, i_sm_W_, in}) + prior :
          affine_transform({i_sm_b_, i_sm_W_, in}));
}
cnn::expr::Expression SoftmaxHinge::CalcLogProb(cnn::expr::Expression & in, cnn::expr::Expression & prior, const vector<Sentence> & ctxt, bool train) {
  return (prior.pg != nullptr ?
          affine_transform({i_sm_b_, i_sm_W_, in}) + prior :
          affine_transform({i_sm_b_, i_sm_W_, in}));
}

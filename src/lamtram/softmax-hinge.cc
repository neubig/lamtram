...
SoftmaxHinge::SoftmaxHinge(const std::string & sig, int input_size, const DictPtr & vocab, ParameterCollection & mod) : SoftmaxBase(sig,input_size,vocab,mod), margin_(1.f) {
...
void SoftmaxHinge::NewGraph(ComputationGraph & cg) {
  i_sm_b_ = parameter(cg, p_sm_b_);
  i_sm_W_ = parameter(cg, p_sm_W_);
}

// Calculate training loss for one word
Expression SoftmaxHinge::CalcLoss(Expression & in, Expression & prior, const Sentence & ngram, bool train) {
...
Expression SoftmaxHinge::CalcLoss(Expression & in, Expression & prior, const std::vector<Sentence> & ngrams, bool train) {
  Expression score = affine_transform({i_sm_b_, i_sm_W_, in});
  if(prior.pg != nullptr) score = score + prior;
...
Expression SoftmaxHinge::CalcProb(Expression & in, Expression & prior, const Sentence & ctxt, bool train) {
  THROW_ERROR("CalcProb not valid for hinge");
  return softmax(affine_transform({i_sm_b_, i_sm_W_, in}));
}
Expression SoftmaxHinge::CalcProb(Expression & in, Expression & prior, const vector<Sentence> & ctxt, bool train) {
  THROW_ERROR("CalcProb not valid for hinge");
  return softmax(affine_transform({i_sm_b_, i_sm_W_, in}));
}
Expression SoftmaxHinge::CalcLogProb(Expression & in, Expression & prior, const Sentence & ctxt, bool train) {
  return (prior.pg != nullptr ?
          affine_transform({i_sm_b_, i_sm_W_, in}) + prior :
          affine_transform({i_sm_b_, i_sm_W_, in}));
}
Expression SoftmaxHinge::CalcLogProb(Expression & in, Expression & prior, const vector<Sentence> & ctxt, bool train) {
  return (prior.pg != nullptr ?
          affine_transform({i_sm_b_, i_sm_W_, in}) + prior :
          affine_transform({i_sm_b_, i_sm_W_, in}));
}

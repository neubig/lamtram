    ...
    for(int t : boost::irange(pad_, (int)sent_trg.size())) {
        ...
        for(int j : boost::irange(0, (int)lms_.size())) {
            if(ensemble_operation_ == "sum")
                i_sms.push_back(lms_[j]->Forward<cnn::Softmax>(sent_trg, t, externs_[j].get(), last_state[j], next_state[j], cg));
            else
                i_sms.push_back(lms_[j]->Forward<cnn::LogSoftmax>(sent_trg, t, externs_[j].get(), last_state[j], next_state[j], cg));
        }
...
Sentence EnsembleDecoder::Generate(const Sentence & sent_src) {
    ...
    for(int sent_len = pad_; sent_len <= size_limit_; sent_len++) {
        ...
        vector<tuple<cnn::real,int,int> > next_beam_id(beam_size_+1, tuple<cnn::real,int,int>(-DBL_MAX,-1,-1));
        ...
        for(int hypid = 0; hypid < (int)curr_beam.size(); hypid++) {
            ...
            // Perform the forward step on all models
            vector<Expression> i_softmaxes;
            for(int j : boost::irange(0, (int)lms_.size())) {
                if(ensemble_operation_ == "sum")
                    i_softmaxes.push_back( lms_[j]->Forward<cnn::Softmax>(sent, sent_len, externs_[j].get(), curr_hyp->GetStates()[j], last_states[hypid][j], cg) );
                else
                    i_softmaxes.push_back( lms_[j]->Forward<cnn::LogSoftmax>(sent, sent_len, externs_[j].get(), curr_hyp->GetStates()[j], last_states[hypid][j], cg) );
            }
            ...
            for(int wid = 0; wid < (int)softmax.size(); wid++) {
                ...
                next_beam_id[bid] = tuple<cnn::real,int,int>(my_score,hypid,wid);
            }
        }

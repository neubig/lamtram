...
std::vector<std::string> Vocabulary::SplitWords(const string & str) {
  char c = ' ';
  ...
  while(1) {
    ...
    if(ptr == begin)
      break;
    result.push_back(string(begin, ptr));
  }
  return result;
}
...
Sentence Vocabulary::ParseWords(const vector<string> & str, int pad, bool add_final) {
  Sentence result(pad, 0);
  for(const auto & s : str)
    result.push_back(WID(s));
  if(add_final)
    result.push_back(0);
  return result;
}

vector<string> Vocabulary::ConvertWords(const Sentence & sent, bool sentend) const {
  vector<string> ret;
  for(WordId wid : sent) {
    if(sentend || wid != 0)
      ret.push_back(WSym(wid));
  }
  return ret;
}
...
string Vocabulary::PrintWords(const vector<string> & sent) const {
  ostringstream oss;
  string filler;

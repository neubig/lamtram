...
EncoderDecoder::EncoderDecoder(
           ...
           ParameterCollection & model) : encoders_(encoders), decoder_(decoder), curr_graph_(NULL) {
...
void EncoderDecoder::NewGraph(ComputationGraph & cg) {
...
std::vector<Expression> EncoderDecoder::GetEncodedState(
                  const SentData & sent_src, bool train, ComputationGraph & cg) {
  ...
  Expression i_decin = affine_transform({i_enc2dec_b_, i_enc2dec_W_, i_combined});
  // Perform transformation
  vector<Expression> decoder_in(decoder_->GetNumLayers() * decoder_->GetLayerMultiplier());
  for (int i = 0; i < decoder_->GetNumLayers(); ++i) {
    decoder_in[i] = pick_range({i_decin}, i * decoder_->GetNumNodes(), (i + 1) * decoder_->GetNumNodes());
...
template
std::vector<Expression> EncoderDecoder::GetEncodedState<Sentence>(
                  const Sentence & sent_src, bool train, ComputationGraph & cg);
template
std::vector<Expression> EncoderDecoder::GetEncodedState<vector<Sentence> >(
                  const vector<Sentence> & sent_src, bool train, ComputationGraph & cg);

Expression EncoderDecoder::BuildSentGraph(const Sentence & sent_src,
...
Expression EncoderDecoder::BuildSentGraph(const std::vector<Sentence> & sent_src,
...
Expression EncoderDecoder::SampleTrgSentences(const Sentence & sent_src,
                                                         const Sentence * sent_trg,   
                                                         int num_samples,
                                                         int max_len,
                                                         bool train,
...
EncoderDecoder* EncoderDecoder::Read(const DictPtr & vocab_src, const DictPtr & vocab_trg, std::istream & in, ParameterCollection & model) {
  int num_encoders;

...
Expression NeuralLM::BuildSentGraph(
                      const Sentence & sent,
                      const Sentence & cache_ids,
                      const float * weight,
                      const ExternCalculator * extern_calc,
                      const std::vector<Expression> & layer_in,
                      float samp_prob,
                      bool train,
                      ComputationGraph & cg,
                      LLStats & ll) {
  ...
  for(auto t : boost::irange(0, slen+1)) {
    ...
    // Run the hidden unit
    Expression i_h_t = builder_->add_input(i_wr_t);
    Expression i_prior;
    ...
    ngram[i] = sent[t];
    Expression i_err = (cache_ids.size() ?
...
Expression NeuralLM::BuildSentGraph(
  ...
  Expression i_wr_start = lookup(cg, p_wr_W_, words);
  vector<Expression> i_wr;
  ...
  for(auto t : boost::irange(0, slen+1)) {
    ...
    if(active_words != sent.size())
      i_err = i_err * input(cg, Dim({1}, sent.size()), mask);
    // If using weights, weigh error for each sentence's word by that sentence's weight
    if (weights)
      i_err = i_err * input(cg, Dim({1}, weights->size()), *weights);
    errs.push_back(i_err);
  }
...
Expression NeuralLM::SampleTrgSentences(
  ...
  if(extern_context_ != 0 && extern_feed_) extern_in = extern_calc->GetEmptyContext(cg);
  // Next, do the computation
  vector<Expression> log_probs, aligns;
  Expression align_sum;
  ...
  for(int t = 0; t < max_len && active_words > 0; t++) {
    ...
    Expression i_err;
    // Perform sampling if necessary
    Expression i_prob = softmax_->CalcProb(i_h_t, i_prior, ctxts, train);
    vector<float> probs = as_vector(i_prob.value());
    if(mask[0]) {
    ...
    i_wr.push_back(lookup(cg, p_wr_W_, words));
    Expression i_log_pick = log(pick(i_prob, words));
    if(active_words != num_samples)
      i_log_pick = i_log_pick * input(cg, Dim({1}, num_samples), mask);
  ...
  return reshape(sum(log_probs), Dim({(unsigned int)num_samples}));
}

void NeuralLM::NewGraph(ComputationGraph & cg) {
...
template <class Sent>
Expression NeuralLM::Forward(const Sent & sent, int t, 
                   const ExternCalculator * extern_calc,
                   bool log_prob,
                   const std::vector<Expression> & layer_in,
                   const Expression & extern_in,
                   const Expression & align_sum_in,
                   std::vector<Expression> & layer_out,
                   Expression & extern_out,
                   Expression & align_sum_out,
                   ComputationGraph & cg,
...
// Instantiate
template
Expression NeuralLM::Forward<Sentence>(
                   const Sentence & sent, int t, 
                   const ExternCalculator * extern_calc,
                   bool log_prob,
                   const std::vector<Expression> & layer_in,
                   const Expression & extern_in,
                   const Expression & sum_in,
                   std::vector<Expression> & layer_out,
                   Expression & extern_out,
                   Expression & sum_out,
                   ComputationGraph & cg,
                   std::vector<Expression> & align_out);
template
Expression NeuralLM::Forward<vector<Sentence> >(
